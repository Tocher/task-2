(function (root) {
    ...
    function retrace(startX,startY,finishX,finishY,maze){
        ...
        function findLowerDistance(x,y,dist){
            if (y-1>=0 && x>= 0 && y-1<maze.length && x<maze[0].length && maze[y-1][x]<distance && maze[y-1][x]!=-1)
                return [x,y-1,distance-1];
            if (y>=0 && x-1>= 0 && y<maze.length && x-1<maze[0].length && maze[y][x-1]<distance && maze[y][x-1]!=-1)
    ...
    function solution(maze, x, y) {
        ...
        if(found){
            maze[y][x]=0;
            var path=retrace(start[0],start[1], finishX, finishY, maze);
